      ...
      <div className="mx-auto max-w-[1680px] px-5 sm:px-8">
        ...
        <div className="mt-10 grid gap-12 lg:grid-cols-12 lg:gap-14">
          ...
          <div className="lg:col-span-5">
            <div className="type-tactical flex items-center gap-3 text-[9px]">
              <span className="text-blood">{drop.index}</span>
              <span className="h-px w-8 bg-iron" />
              <span className="text-ash">{drop.line}</span>
            </div>
            ...
            <div className="type-tactical mt-5 text-[10px] text-blood">
              <Scramble text={drop.battlecry} />
            </div>

            <p className="mt-7 text-sm leading-relaxed text-smoke sm:text-base">{drop.story}</p>

            <dl className="mt-9 grid gap-px overflow-hidden border border-iron/70 bg-iron/70 sm:grid-cols-2">
              {drop.specs.map((spec) => (
                <div key={spec.label} className="bg-void p-5">
            ...
            <p className="type-tactical mt-6 text-[9px] leading-relaxed text-ash">
        ...
        <section className="mt-24 border-t border-iron/70 pt-14 sm:mt-32">
          <div className="mb-10 flex items-center gap-4">
            <span className="type-tactical text-[10px] text-blood">RESTO DO ARSENAL</span>
            <span className="h-px flex-1 bg-iron" />
          </div>

          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {others.map((other) => (
              <DropCard key={other.slug} drop={other} />
            ))}
          </div>
        </section>
      </div>
